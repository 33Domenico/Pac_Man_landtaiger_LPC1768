...
/*----------------------------------------------------------------------------
	PacMan Display Function
 *----------------------------------------------------------------------------*/

void draw_maze(uint8_t (*maze)[20]) {
    // Offset per centrare il labirinto sullo schermo
    const uint16_t OFFSET_X = (MAX_X - (COLS * CELL_SIZE)) / 2;
    const uint16_t OFFSET_Y = (MAX_Y - (ROWS * CELL_SIZE)) / 2;
  ...
  int row;  
	...
	int j;
		for (row = 0; row < ROWS; row++) {
            ...
            uint16_t x = OFFSET_X + col * CELL_SIZE;
            uint16_t y = OFFSET_Y + row * CELL_SIZE;
            
            switch (maze[row][col]) {
              case 1://se è un muro
               
							// Disegna la linea superiore solo se non c'è un muro sopra
                if (row == 0 || maze[row-1][col] != 1) {
                    LCD_DrawLine(x, y, x + CELL_SIZE, y, Blue);
                }
                
                // Disegna la linea inferiore solo se non c'è un muro sotto
                if (row == ROWS-1 || maze[row+1][col] != 1) {
                    LCD_DrawLine(x, y + CELL_SIZE, x + CELL_SIZE, y + CELL_SIZE, Blue);
                }
                
                // Disegna la linea sinistra solo se non c'è un muro a sinistra
                if (col == 0 || maze[row][col-1] != 1) {
                    LCD_DrawLine(x, y, x, y + CELL_SIZE, Blue);
                
								}
                
                // Disegna la linea destra solo se non c'è un muro a destra
                if (col == COLS-1 || maze[row][col+1] != 1) {
										if (x+CELL_SIZE >= MAX_X){
											LCD_DrawLine(MAX_X-1, y, MAX_X-1, y + CELL_SIZE, Blue);
										}else{
                    LCD_DrawLine(x + CELL_SIZE, y, x + CELL_SIZE, y + CELL_SIZE, Blue);
										}
                }
							...
							case 2: // Pillola standard (punto più piccolo)
                    LCD_SetPoint(x + CELL_SIZE/2, y + CELL_SIZE/2, White);
                    break;
                    
                case 3: // Pillola di potenza (punto più grande)
                    for(i = -1; i <= 1; i++) {
                        for(j = -1; j <= 1; j++) {
                            LCD_SetPoint(x + CELL_SIZE/2 + i, y + CELL_SIZE/2 + j, White);
                        }
                    }
                    break;
...
void draw_PacMan(uint16_t x, uint16_t y, int radius) {
    int i, j;
    for (i = -radius; i <= radius; i++) {
        for (j = -radius; j <= radius; j++) {
            // Disegna solo i punti all'interno del cerchio
            if (i * i + j * j <= radius * radius) {
                LCD_SetPoint(x + CELL_SIZE / 2 + i, y + CELL_SIZE / 2 + j, Yellow);
            }
        }
    }
}
...
void clear_cell(uint16_t row, uint16_t col) {
    const uint16_t OFFSET_X = (MAX_X - (COLS * CELL_SIZE)) / 2;
    const uint16_t OFFSET_Y = (MAX_Y - (ROWS * CELL_SIZE)) / 2;
    uint16_t x = OFFSET_X + col * CELL_SIZE;
    uint16_t y = OFFSET_Y + row * CELL_SIZE;
    int i;
		int j;
    // Pulisce la cella disegnando un rettangolo nero  
...
void draw_cell(uint16_t row, uint16_t col, uint8_t type) {
    const uint16_t OFFSET_X = (MAX_X - (COLS * CELL_SIZE)) / 2;
    const uint16_t OFFSET_Y = (MAX_Y - (ROWS * CELL_SIZE)) / 2;
    uint16_t x = OFFSET_X + col * CELL_SIZE;
    uint16_t y = OFFSET_Y + row * CELL_SIZE;
...
//funzione per conversione tipi interi in stringhe
void uint_to_str(uint32_t num, char* str) {
    char temp[11];  // Max lunghezza per uint32_t
    int i = 0;
    
    // Gestisce il caso dello zero
    if (num == 0) {
        str[0] = '0';
        str[1] = '\0';
        return;
    ...
    // Copia al contrario nel buffer finale
    int j;
    for (j = 0; j < i; j++) {
        str[j] = temp[i - 1 - j];
    }
...
void display_lives(void) {
    const uint16_t OFFSET_X = (MAX_X +5 - (COLS * CELL_SIZE)) / 2; // Offset orizzontale del labirinto
    const uint16_t START_Y = (MAX_Y - 40); 
    const uint16_t SPACING = CELL_SIZE + 5; // Spazio tra le vite
		int i;
    for (i = 0; i < lives; i++) {
        uint16_t x = OFFSET_X + i * SPACING;
        draw_PacMan(x, START_Y, 4); //le vite sono rappresentate come pac_man
    }
}


void display_game_stats(void) {
    char score_text[25] = "Your score: ";  // Buffer più grande per sicurezza
		uint_to_str(score, score_text + 11);   // +11 per saltare "Your score: "
    GUI_Text(5, 5, (unsigned char*)score_text, White, Black);
		display_lives();
...
void display_game_over(void){
	...
	if(game_state==1){
	 GUI_Text(95, 120, (unsigned char*)"Victory!", Yellow, Black);
	}else{
	  GUI_Text(90, 120, (unsigned char*)"Game Over!", Red, Black);
	}
}
...
void display_countdown(void){
	// pulisci il tempo precedente(saltando la parte Time:)
	GUI_Text(215, 5, (unsigned char*)"   ", Black, Black); 
	
	char time_text[20]="Time:";
	char countdown_str[3];
	uint_to_str(countdown, countdown_str);
    strcat(time_text, countdown_str);
    GUI_Text(180, 5, (unsigned char*)time_text, White, Black);
}
